...
def read_inp(f_name):
	...
	for i in range(len(lines)):
		...
		if "kpt_file" in lines[i]:
			w = lines[i+1].split()
			kpt_fn = eval(w[0])
	return alat, A_uc, sc, kpt_fn
...
def findkuc(kpt, b1, b2, b1_uc, b2_uc, mrk):
	...
	kpG = []
	kpG_all = []
	for n1 in range(-75,76):
		for n2 in range(-75,76):
...
alat, A_uc, sc, kpts_fn  = read_inp("ksc2kuc.inp")
...
mrk = '^'
kpts_sc = read_kpt_sc(kpts_fn,B_sc)
