...
def main():
    ...
    ########################
    sc_eps_path = '../'
    uc_eps_path = './epsilon_data_top/'
    #sc_eps_path = '../sc/3.1-chi-separate/'
    #uc_eps_path = '../uc/3.1-chi/'
    change_nmtx = True
    # kuc_map_crys : real(nqsc, size_sc, 3)
    quc_map_crys = np.load('/pscratch/sd/w/wkim94/Moire/TBG/1.08/CC.KC-full.CH.rebo/Unitcell_3x3x1/Top/bfold/kuc_map_crys.npy')
    ...
    # Change nmtx
    if change_nmtx:
        chi_uc_nmtx = np.loadtxt('/pscratch/sd/w/wkim94/Moire/TBG/1.08/CC.KC-full.CH.rebo/Unitcell_3x3x1/Top/epsilon_4ry/chimat.nmtx.txt', dtype=np.int32)
        chi0_uc_nmtx = np.loadtxt('/pscratch/sd/w/wkim94/Moire/TBG/1.08/CC.KC-full.CH.rebo/Unitcell_3x3x1/Top/epsilon_4ry/chi0mat.nmtx.txt',dtype=np.int32)
        if comm.rank == 0:
            print("Change nmtx!")
            os.sys.stdout.flush()
